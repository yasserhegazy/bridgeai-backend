...
from app.schemas.project import ProjectCreate, ProjectRejectionRequest, ProjectUpdate
...
class TestInputValidation:
    ...
    def test_user_password_validation(self):
        ...
        with pytest.raises(ValidationError) as exc:
            UserCreate(
                full_name="Test User", email="test@example.com", password="short"
            )
        ...
        with pytest.raises(ValidationError) as exc:
            UserCreate(
                full_name="Test User", email="test@example.com", password="Password"
            )
        assert "digit" in str(exc.value).lower()

        # Valid password
        user = UserCreate(
            full_name="Test User", email="test@example.com", password="ValidPass123"
        )
    ...
    def test_project_name_validation(self):
        ...
        # Invalid characters
        with pytest.raises(ValidationError) as exc:
            ProjectCreate(name="Project<script>", description="Test", team_id=1)
        assert "can only contain" in str(exc.value).lower()

        # Valid name
        project = ProjectCreate(
            name="My-Project_2024", description="Test description", team_id=1
        )
    ...
    def test_project_description_xss_prevention(self):
        ...
        for xss_input in xss_inputs:
            with pytest.raises(ValidationError) as exc:
                ProjectCreate(name="Test Project", description=xss_input, team_id=1)
    ...
    def test_team_description_xss_prevention(self):
        ...
        with pytest.raises(ValidationError) as exc:
            TeamCreate(name="Test Team", description="<script>alert('xss')</script>")
        assert "invalid content" in str(exc.value).lower()
    ...
    def test_length_constraints(self):
        ...
        with pytest.raises(ValidationError):
            ProjectCreate(name="x" * 257, description="Test", team_id=1)

        # Description too long
        with pytest.raises(ValidationError):
            ProjectCreate(name="Test", description="x" * 2001, team_id=1)
    ...
    def test_positive_integer_validation(self):
        ...
        with pytest.raises(ValidationError):
            ProjectCreate(name="Test", description="Test", team_id=0)  # Must be > 0

        with pytest.raises(ValidationError):
            ProjectCreate(name="Test", description="Test", team_id=-1)  # Must be > 0
...
class TestSanitization:
    ...
    def test_whitespace_trimming(self):
        ...
        user = UserCreate(
            full_name="  John Doe  ", email="test@example.com", password="ValidPass123"
        )
    ...
    def test_email_normalization(self):
        ...
        invitation = InvitationCreate(email="  TEST@EXAMPLE.COM  ", role="member")
