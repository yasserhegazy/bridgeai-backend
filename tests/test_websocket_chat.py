...
def test_websocket_message_persistence(
    client: TestClient, websocket_test_data, db: Session
):
    ...
    messages_before = db.query(Message).filter(Message.session_id == session_id).count()
    ...
    messages_after = db.query(Message).filter(Message.session_id == session_id).count()
...
def test_websocket_broadcast_to_multiple_clients(
    client: TestClient, websocket_test_data
):
...
def test_websocket_invalid_sender_type(client: TestClient, websocket_test_data):
    ...
    with client.websocket_connect(
        ...
        message_data = {"content": "Test message", "sender_type": "invalid_type"}
        websocket.send_json(message_data)
...
def test_websocket_empty_content(client: TestClient, websocket_test_data, db: Session):
    ...
    with client.websocket_connect(
        ...
        message_data = {"content": "   ", "sender_type": "client"}
        websocket.send_json(message_data)
...
def test_websocket_unauthorized_project_access(
    client: TestClient, websocket_test_data, db: Session
):
