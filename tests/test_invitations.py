...
class TestInvitationRetrieval:
    ...
    def test_list_team_invitations(self, client: TestClient, client_auth_headers: dict):
        ...
        # List invitations
        response = client.get(
            f"/api/teams/{team_id}/invitations", headers=client_auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
    ...
    def test_member_cannot_list_invitations(
        ...
        response = client.get(
            f"/api/teams/{team_id}/invitations", headers=another_client_auth_headers
        )
        assert response.status_code == 403
...
class TestInvitationAcceptance:
    ...
    def test_accept_invitation_success(
        ...
        # Accept invitation
        response = client.post(
            f"/api/invitation/{token}/accept", headers=another_client_auth_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestInvitationNotifications:
    ...
    def test_accepting_invitation_notifies_owner(
        ...
        acceptance_notif = next(
            (n for n in notifications if "joined" in n.message.lower()), None
        )
